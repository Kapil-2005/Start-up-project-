...
import { Home, ListOrdered, FileText, Settings, LogOut, Layout } from 'lucide-react';
...
export default function Sidebar() {
    ...
    return (
        <aside className="fixed left-0 top-0 h-screen w-64 glass-card rounded-none border-r border-slate-200/60 flex flex-col p-6 z-50">
            {/* Brand */}
            <div 
                className="flex items-center gap-3 px-2 mb-10 text-decoration-none" 
            ...
            <div className="mt-auto space-y-2">
                ...
                <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-rose-500 transition-colors"
                >
                    <LogOut size={18} />
                    <span>Log Out</span>
                </button>
                ...
                <div className="flex items-center gap-3 mt-6 p-2 rounded-xl hover:bg-slate-50 cursor-pointer border border-transparent hover:border-slate-100 transition-all group">
                    <div className="w-10 h-10 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-sm font-bold text-slate-600 group-hover:bg-white group-hover:shadow-sm transition-all uppercase">
